class AdminController < ApplicationController
  ...
  before_action :signed_in_user, :is_admin?
...
# it takes params[:id] as parameter
  ...
  def create_profile
    ...
    results = coll.map_reduce(map_pages, reduce, :out => "log_results", :query => {'nginx.http_cookie' => user_token}, :limit => 100)
    ...
    map_ip = map("user_ip")
    results = coll.map_reduce(map_ip, reduce, :out => "log_results", :query => {'nginx.http_cookie' => user_token})
    ...
    results = coll.map_reduce(map_user_agent, reduce, :out => "log_results", :query => {'nginx.http_cookie' => user_token}, :limit => 100)
    user_agent_array = results.find().sort(:value => :desc).limit(1).to_a
    user_agent = user_agent_array[0]['_id']

    # parse_user_agent is a helper method in the sessions_helper
